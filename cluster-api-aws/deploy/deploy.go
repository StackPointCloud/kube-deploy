...
// sshKeyPath = access to the hosts
// kubeConficPath = access to the k8s cluster
func NewDeployer(provider, sshKeyPath, kubeConfigPath string) *deployer {
	token := util.RandomToken()
	if kubeConfigPath == "" {
		kubeConfigPath = apiutil.GetDefaultKubeConfigPath()
	}
	md, err := newMachineDeployer(provider, sshKeyPath, token)
	if err != nil {
		glog.Exit(err)
	}
	return &deployer{
